...
func InitSchema(ctx context.Context, pool *pgxpool.Pool) error {
	...
	// ivf indexing at the end after inserting data since it needs all data inserted
	stmt :=
...
func Retrieve(ctx context.Context, pool *pgxpool.Pool, query string, k int) ([]Result, error) {
	...
	// added newly to observe how indexing effects
	fmt.Println("\n--- Query plan ---")
	rows, err := pool.Query(ctx,
		`EXPLAIN ANALYZE SELECT name FROM docs
	...
	// specifically for ivfflat otherwise it will only compare from default = 1 probe
	if err != nil {
		panic(err)
	}
