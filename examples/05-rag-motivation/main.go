...
import (
	...
	"github.com/veerLakshya/ai-examples/foundation/openai"
)
...
func Ask(ctx context.Context, prompt string) (response string, approxTokens int, err error) {
	req := openai.ChatRequest{
		Model:    ModelName,
		Messages: []openai.Message{{Role: "user", Content: prompt}},
	}

	resp, err := client.PostJSON[openai.ChatResponse](ctx, ChatUrl, req)
	if err != nil {
		return "", 0, err
	}
	...
	return resp.Choices[0].Message.Content, len(prompt) / 4, nil
}

func AskStreaming(ctx context.Context, prompt string) (string, int, error) {
	req := openai.ChatRequest{
		Model:    ModelName,
		Messages: []openai.Message{{Role: "user", Content: prompt}},
		Stream:   true,
	...
	err := client.StreamSSE(ctx, ChatUrl, req, func(cc openai.ChatChunk) error {
		if len(cc.Choices) == 0 {
			return nil
		}
	...
	return sb.String(), len(prompt) / 4, err
}
...
func main() {
	...
	//----------------------------------------------------
	// Run 1: without context
	fmt.Printf("--- Cold ---:\n\n")
	coldPrompt := Question
	coldAns, coldTokens, err := AskStreaming(ctx, coldPrompt)
	if err != nil {
		panic(err)
	}
	_ = coldAns
	...
	//----------------------------------------------------
	// Run 2: single doc
	fmt.Printf("--- Single Doc ---:\n\n ")
	singleDocPrompt := fmt.Sprintf("Context:\n %s\n Question: %s", Docs[0].Body, Question)
	singleDocAns, singleDocTokens, err := AskStreaming(ctx, singleDocPrompt)
	if err != nil {
		panic(err)
	}
	_ = singleDocAns
	...
	allDocsPrompt := fmt.Sprintf("Context: \n\n %s\n Question: %s", sb, Question)
	AllDocAns, allDocsTokens, err := AskStreaming(ctx, allDocsPrompt)
	if err != nil {
		panic(err)
	}
	_ = AllDocAns
